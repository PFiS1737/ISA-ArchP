...
fn parse_line(line_no: usize, line: &str) -> anyhow::Result<Vec<Line<'_>>> {
    ...
    let (remain, instr) = instr(line_no, rest)
        .map_err(|e| anyhow!("Error parsing line {}: '{}': {}", line_no, line, e))?;

    if !remain.is_empty() {
        bail!("Unexpected content after line {}: '{}'", line_no, remain);
    }
...
mod tests {
    ...
    fn parse_ok(input: &str) -> Source<'_> {
